...
class FakeElement extends EventTarget {
    ...
    constructor() {
        ...
        this.dataset = new Proxy({}, {
            get(_target, key) { return element.attributes.get(attributeName(key)); },
    ...
    appendChild(child) { this.children.push(child); child.parent = this; child.isConnected = true; child.connectedCallback?.(); }
    remove() {
        if (this.parent) this.parent.children = this.parent.children.filter(child => child !== this);
        this.isConnected = false;
        this.disconnectedCallback?.();
    }
    getBoundingClientRect() { return {left: 0, top: 0, right: 100, bottom: 140, height: 140}; }
...
test("PlayingCard properties and pointer lifecycle", async t => {
    const originals = new Map(["HTMLElement", "customElements", "document", "window"].map(name => [name, globalThis[name]]));
    const registered = new Map();
    globalThis.HTMLElement = FakeElement;
    globalThis.customElements = {
        define(name, Type) { registered.set(name, Type); },
    ...
    try {
        ...
        await t.test("shares model properties and keeps derived values read-only", () => {
            ...
            for (const name of ["value", "suit", "rank", "score", "isDragging"]) {
                assert.throws(() => { element[name] = 0; }, TypeError);
            }
        ...
        await t.test("updates are validated before changing identity or rotation", () => {
            ...
            for (const source of [{value: "2", suit: "clubs", rotation: NaN}, {value: "joker", suit: "hearts"}]) {
        ...
        await t.test("rotation validates without losing its previous value and can use CSS", () => {
            ...
            for (const value of [NaN, Infinity, "20"]) {
                assert.throws(() => { element.rotation = value; }, /finite number/);
                assert.equal(element.rotation, 15);
            }
            element.rotation = -30;
        ...
        await t.test("face setters, attributes, click, and keyboard update accessibility", () => {
            ...
            for (const property of ["isFaceUp"]) {
                assert.throws(() => { element[property] = "false"; }, /boolean/);
            }
            element.remove();
        });
        ...
        await t.test("each card uses its own destination and ignores a release outside it", () => {
            ...
            let leftDrops = 0;
            let rightDrops = 0;
            left.addEventListener("card_drop", () => { leftDrops += 1; });
            right.addEventListener("card_drop", () => { rightDrops += 1; });
            const first = PlayingCard.create(model, left);
            const second = PlayingCard.create(model, right);
            document.body.appendChild(first);
            document.body.appendChild(second);
            for (const [element, x] of [[first, 30], [second, 30], [second, 250]]) {
                pointer(element.querySelector(".playing-card-drag-handle"), "pointerdown");
